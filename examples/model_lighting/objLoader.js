ObjLoader = (function() {
    ...
    function parseObjs(objFileText, cb) {
      ...
      for (i = 0; i < lines.length; ++i) {
          ...
          if (line.match(/^#.*\r?$/)) {
            ...
            if (currentObj && vNormals.length != vCoords.length) {
                console.log("OH NOES, NOT ENOUGH NORMALS OR VERTEXES IN OBJ " + currentObj);
            }
            ...
            vNormals = currentObj.vertexNormals;
            ...
            // push normals
            for (j = 0; j < 3; ++j) {
                tempVec = vNormalStore[parseInt(faceParts[j * 3 + 2]) - 1];
                vNormals.push(parseFloat(tempVec[0]));
                vNormals.push(parseFloat(tempVec[1]));
                vNormals.push(parseFloat(tempVec[2]));
            }
          }
      }
    ...
    function objToModel(obj, materials, gl) {
      ...
      return new Model(vertexBuffer, texCoordBuffer, vNormalBuffer, objMaterial.textureId, obj.vertexCoordinates.length / 3);
    }
